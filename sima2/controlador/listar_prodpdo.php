...
if(!$consulta){
    ...
    $consulta=(isset($_POST['buscar']))?$_POST['buscar']:"";
    $sentencia=$pdo->prepare("SELECT id_item , nombre_prod , id_marcas , fecha_vencimiento , cantidad_prod FROM producto where id_item= :consulta or nombre_prod= :consulta or id_marcas = :consulta");
    $sentencia->bindParam(':consulta',$consulta);
    $sentencia->execute();
    $listaProductos=$sentencia->fetchAll(PDO::FETCH_ASSOC);
    if(empty($listaProductos)){?>
                            <script> alertify.error("el producto que busca no existe");</script>
                        <?php
